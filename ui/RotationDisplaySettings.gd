...
#region value_changed signals
	$X.value_changed.connect(func():
		...
		ProjectManager.current_project.model.display.firstperson_righthand.rotation.x = x
	)
	$Y.value_changed.connect(func():
		if not ProjectManager.current_project:
			return
		
		ProjectManager.current_project.model.display.firstperson_righthand.rotation.y = y
	)
	$Z.value_changed.connect(func():
		if not ProjectManager.current_project:
			return
		
		ProjectManager.current_project.model.display.firstperson_righthand.rotation.z = z
	)
#endregion
...
func _add_keyframe(axis: String):
	...
	var path = NodePath(project.get_path_to(project.model).get_concatenated_names() + ":display:firstperson_righthand:rotation:" + axis)
	...
	match axis:
		'x':
			value = display.rotation.x
		'y':
			value = display.rotation.y
		'z':
			value = display.rotation.z
